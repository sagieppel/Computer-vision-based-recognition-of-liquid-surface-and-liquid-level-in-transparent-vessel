function [ Score] = MatchEllipse10e(Is,Iel,~,Ycnt,~) 
...
ScoreUp=0;
  ...
  ScoreUp=double(ScoreUp)+double(p(f)*Igr(p(f))*cos((dIs(p(f))-dIt(p(f))))/Is(p(f)));% the score value of x,y point  is increase or decrease according to how close the value of the gradient direction and image of this point the gradient difference could be between zero in which the value increase by 1 and 2pi in which the value decrease by one
...
ScoreDn=0;
   for f=1:length(p)%
    ...
    ScoreDn=double(ScoreDn)+double(p(f)*Igr(p(f))*cos((dIs(p(f))-dIt(p(f))))/Is(p(f)));% the score value of x,y point  is increase or decrease according to how close the value of the gradient direction and image of this point the gradient difference could be between zero in which the value increase by 1 and 2pi in which the value decrease by one
