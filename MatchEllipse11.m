function [ Score] = MatchEllipse11(Ir,Iellipse,Resly,Ycnt,Ierea,Fract)   
...
%=======================================working mode===========================================================================
%find the relative intensity difference along  each point in the border and find the value that Fract of  the
...
% Fract: % The score is the relative intensity difference that Fract (0<Fract<1) of the points have higher values.
...
%Hightd_To_Width_Ellipse_Ratio=0.3; recomanded value
  %Paper Table Entries 1
if (nargin<6) Fract=0.65; end;% The score is the relative intensity difference that Fract (0<Fract<1) of the points have higher values.
...
sumpix=double(0);%number used border points
 ...
 IRatioArray=double(ones(length(x),1)*10^5); % array of relative intsnisty change across each border point
...
for f=1:length(x)% scann all points on the ellipse contour but use only the upper part of th ellipse
    if y(f)>=Ycnt% upper half  of ellipse
        ...
        %.......................find relative difference in score and add to the array...............................................................................................
        if npixup>0 &&   npixdn>0 % the average itnensity above and below borders
           ...
           sumpix=sumpix+1; 
            ...
            IRatioArray(sumpix)=double(up-dn)/double(max([up dn 1])); % write  relative difference in intensity along this border point
...
%=======================================calculate score of lower ellipse parts========================================================================================================
 ...
 %---------------------check the  score of the upper part of the ellipse--------------------------------------------------------
  if sumpix>15 %check if enough points were used to make reliable prediction and if so normalize the score according to the number of points used
         IRatioArray = sort(IRatioArray);%Get the relative difference that  Fract of the surface point are above it
 ...
 IRatioArray=ones(length(x),1)*10^5; % array of relative intsnisty change across each border point
