function [ Score] = MatchEllipse9e(Is,Iel,~,Ycnt,~) 
%same as method 9  but different calculating of score in line 37-38
...
%=======================================working mode===========================================================================
%similar to mathc ellipse 15 (hough transform)
...
% The  local score of each point of the curve was taken as zero if the difference beween the gradient 
%angel in this point and the curve normal was larger then 2Pi/10.
%Otherwise the local score of the point was taken as the intensity of point in the edge image (0 or 1 in canny).
...
dIs=gradient_direction_full(Is);% gradient dirction map 
dIt=gradient_direction_full(imfill(Iel,4,'holes'));% gradient dirction map   
...
if (abs(dIs(p(f))-dIt(p(f)))<2*pi/(2*NUMBEROFBINS)) ScoreUp=ScoreUp+Is(p(f)); end;% the score value of x,y point  is increase or decrease according to how close the value of the gradient direction and image of this point the gradient difference could be between zero in which the value increase by 1 and 2pi in which the value decrease by one
  
    end;
    ScoreUp=abs(ScoreUp)/length(p);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Scan upper part of ellipse POINT BY POINT and  get it's score %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
IelDn=Iel;
IelDn(1:Ycnt-1,:)=0;
...
ScoreDn=0;
   for f=1:length(p)%
         ...
         %   ScoreDn=ScoreDn+IelDn(p(f))*Is(p(f))*(1-1*abs(mod((dIs(p(f))-dIt(p(f)))/pi(),0.5)));% the score value of x,y point  is increase or decrease according to how close the value of the gradient direction and image of this point the gradient difference could be between zero in which the value increase by 1 and 2pi in which the value decrease by one
            if (abs(dIs(p(f))-dIt(p(f)))<2*pi/(2*NUMBEROFBINS)) ScoreDn=ScoreDn+Is(p(f)); end;% the score value of x,y point  is increase or decrease according to how close the value of the gradient direction and image of this point the gradient difference could be between zero in which the value increase by 1 and 2pi in which the value decrease by one
   
    end;
    ScoreDn=abs(ScoreDn)/length(p);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [ Is ] = gradient_direction_full( i3 )

%return absolute gtadient direction from -pi to  pi for each point in the
%image (sobel mode)
h = [1;-1];%fspecial('sobel');%
i3=double(i3);%convert image to double to accumolate for negative values
%-------------------------------------------------------------------
Dy=imfilter(i3,h,'same');%y first derivative  sobel mask
Dx=imfilter(i3,h','same');% z sobel first derivative sobel mask
%Is=atan2(Dy,Dx)+pi();
Is=atan2(Dy,Dx);
%--------------------show the image-----------------------------------------------
%{
